...
class ChatbotBrain {
  ...
  buildNeuralCandidate(message) {
    ...
    // Require a stronger match from neural prototypes to avoid noisy/over-aggressive recalls
    if (!best || best.cosine < 0.65) {
      return null;
    }
  ...
  findBestMemory(message, sessionId = null) {
    ...
    // Require a higher similarity before replaying an old memory to reduce
    // inappropriate or noisy memory replays on ambiguous user input.
  ...
  isAbusive(message) {
    ...
    const normalized = this.normalize(String(message));
    ...
    const re = new RegExp(abusePatterns.join('|'), 'i');
...
A key lesson here is that understanding improves when we compare multiple viewpoints and update beliefs based on stronger evidence.`
  ...
  buildCandidates(message, concepts, sessionId) {
    ...
    // Provide a concise, non-self-referential fallback instead of verbose
    // training/status messaging that can feel noisy in casual chat.
    if (intent === 'emotion') {
      candidates.push({
        text: 'I hear you. If you want to talk about it, tell me more; otherwise ask a specific question and I will help.',
        source: 'generated',
        baseScore: 0.7
      });
    } else {
      candidates.push({
        text: 'I don\'t have a concise answer right now. Ask a focused question (for example: "define photosynthesis") or provide a short lesson to teach me.',
        source: 'generated',
        baseScore: 0.42
      });
    }
